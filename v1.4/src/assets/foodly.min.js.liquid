...
$.ajax({type:"post",url:"/cart/update.js",data:'updates['+variantId+']='+quantity,dataType:'json',success:function(data){$this.doUpdateDropdownCart(data,false);}});});$forms.submit(function(e){e.preventDefault();$(this).closest('.product__visuals').addClass('product-in-cart');var $target=$(e.target);var variantId=$target.find('[name=id]').val();var quantity=$target.find('input[name=quantity]').val()||1;$this.doAjaxAddToCart(variantId,quantity,$(this));});}else{$forms.submit(function(e){e.preventDefault();var $target=$(e.target);var variantId=$target.find('[name=id]').val();var quantity=$target.find('input[name=quantity]').val()||1;$this.doAjaxAddToCart(variantId,quantity,$(this));});}},doAjaxAddToCart:function(variantId,quantity,$_this_){if($_this_.hasClass('wishlist-add-cart')){var $elementForInsert=$('.table-wrapper');}else{var $elementForInsert=$_this_.find('.js-add-to-card');}
...
this.initChangeQuantity('.cart');this.cache.$cart.find('.js-cart-item-remove').click(function(e){e.preventDefault();var productId=$(this).parents('.cart-list__item').attr('id');productId=productId.match(/\d+/g);{%if settings.collection_add_cart_thumb%}
$this.updateForms(cart,productId);{%endif%}
Shopify.removeItem(productId,function(cart){foodly.doUpdateDropdownCart(cart,true);});$this.initChangeQuantity('.cart');});}else{$cartForm.addClass('display-none');$cartEmpty.removeClass('display-none');}
{%if settings.show_multiple_currencies%}
if(this.cache.$cart.find('.cart-total__amount .money').attr('data-currency')){this.cache.$cart.find('.cart-total__amount .money').removeAttr('data-currency-'+Currency.currentCurrency.toLowerCase());this.cache.$cart.find('.cart-total__amount .money').removeAttr('data-currency');}
...
{%endif%}},initChangeQuantity:function(form){var $form=$(form);var forms=document.querySelectorAll(form);if(forms.length===0){return 0;}
...
var $firstName=$form.find('[name*=first_name]');var $lastName=$form.find('[name*=last_name]');var $email=$form.find('[name*=email]');var $password=$form.find('[name*=password]');var $passwordConfirm=$form.find('[name*=\\[password_confirmation\\]]');var $message=$form.find('[name*=body]');var $fullName=$form.find('[name*=\\[name\\]]');var $author=$form.find('[name*=author]');var $address1=$form.find('[name*=address1]');var $city=$form.find('[name*=city]');var $zip=$form.find('[name*=zip]');var errorMsgName="{{ 'general.form_popup_errors.name_html' | t }}";var errorMsgLastName="{{ 'general.form_popup_errors.last_name_html' | t }}";var errorMsgEmail="{{ 'general.form_popup_errors.email_html' | t }}";var errorMsgPassword="{{ 'general.form_popup_errors.password_html' | t }}";var errorMsgMessage="{{ 'general.form_popup_errors.message_html' | t }}";var errorMsgFullname="{{ 'general.form_popup_errors.fullname_html' | t }}";var errorMsgAuthor="{{ 'general.form_popup_errors.comment_author_html' | t }}";var errorMsgAddress1="{{ 'general.form_popup_errors.city_html' | t }}";var errorMsgCity="{{ 'general.form_popup_errors.city_html' | t }}";var errorMsgZip="{{ 'general.form_popup_errors.zip_html' | t }}";var counter=0;function initErrorMsg(input,msg){for(var i=0,max=$inputs.length;i<max;i++){if(input.is($inputs[i])){counter=i+1;}}
...
if(y>200){$parallaxContainer.addClass('parallax-move');}});});},setParallaxCoords:function($parallaxBack,$parallaxBase,$header){if($header.length===0||$parallaxBack.length===0||$parallaxBase.length===0){return 0;}
